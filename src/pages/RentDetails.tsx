...
const RentDetails = () => {
  ...
  if (!property) {
    return (
      <div className="text-center text-gray-500 py-24">Property not found</div>
    );
  }
  ...
  const Arrow = ({
    ...
    <div
      onClick={onClick}
      className={`absolute hidden md:block top-1/2 ${
        dir === "right" ? "right-4" : "left-4"
      } z-10 -translate-y-1/2 bg-white dark:bg-black p-2 rounded-full shadow cursor-pointer`}
    >
      {dir === "right" ? <ChevronRight size={40} /> : <ChevronLeft size={40} />}
    </div>
  );
